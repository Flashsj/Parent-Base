	...
	void tick()
	{
		if (g_Opened)
		{
			...
			// Draw options.
			{
				...
				{
					for (int i{}; i < current_sub->m_Options.size(); ++i)
					{
						if (i == current_sub->m_CurrentOptionIndex)
						{
							GRAPHICS::DRAW_RECT((x_pos - (width / m_ChildParentSubs.size()) / 2) + width / 2, y_pos + ((i + 1) * option_height) - (option_height - parent_bar_height), width, option_height, text_colour_selected.r, text_colour_selected.g, text_colour_selected.b, text_colour_selected.a, 1); // This is very confusing so dont bother reading it.
							...
							if (current_sub->m_Options[i]->m_Type == OptionType::RegularOption)
							{
								draw_left_text(current_sub->m_Options[i]->m_Name.c_str(),
									...
									y_pos + parent_bar_height / 2 + (i * option_height),
									option_text_size,
									Font::ChaletLondon,
									background_colour_selected,
									false, false);
						...
						{
							GRAPHICS::DRAW_RECT((x_pos - (width / m_ChildParentSubs.size()) / 2) + width / 2, y_pos + ((i + 1) * option_height) - (option_height - parent_bar_height), width, option_height, text_colour.r, text_colour.g, text_colour.b, text_colour.a, 1); // This is very confusing so dont bother reading it.
